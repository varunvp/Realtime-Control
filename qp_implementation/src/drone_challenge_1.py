...
def other_drone_positions_cb(data):
    ...
    dest_point.point = data.center
    p = tf_buff.transform(dest_point, args.namespace+"/base_link", timeout=rospy.Duration(0.05))
    
    other_drone = [[p.point.x], [p.point.y], [data.radius], [data.velocity.x], [data.velocity.y]]
...
def gps_local_cb(data):
    ...
    x_current = data.pose.position.x + x_seperation
    y_current = data.pose.position.y + y_seperation
    ...
    br.sendTransform(t)
    # br2.sendTransform((pos.x, pos.y, pos.z), (0, 0, 0, 1), rospy.Time.now(), args.namespace+"/base_scan", "local_origin")
...
def main():
    global x_home_recorded, z_home_recorded, args, counter, lin_vel_lim, z_destination, x_seperation, y_seperation, pub, sleep_flag
    global kp, kb, cont, gps_rate, n, t, cached_var, other_drone, x_velocity_des, y_velocity_des, is_leader, last_time, prev_x
    xAnt = yAnt = 0
    ...
    # rospy.Subscriber(mavros.get_topic('global_position', 'global'), NavSatFix, home_pos_cb)
    ...
    while not rospy.is_shutdown():
        ...
        current_position.radius = 1
        current_position.true_radius = 1.5
        ...
        if len(other_drone) > 0 and len(other_drone[0]) > 0:
            ...
            for i in range(0, 2):
                ...
                obs_pose.header.frame_id = args.namespace + '/base_link'
            ...
            obs_path.header.frame_id = args.namespace + '/base_link'
        ...
        if(is_leader):
            if(math.fabs(x_current - final_pose[0]) < 0.5 and math.fabs(y_current - final_pose[1]) < 0.5):
                ...
                other_drone_target.pose.position.x = x_current + 5
