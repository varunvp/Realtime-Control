	...
	if obstacles != None and len(obstacles) != 0:
		...
		while(d > 0.1 and iterations < 10):
			...
			for j in range(0, len(obstacles[0])):
				...
				total_time = - nsteps * interval

				x_proj, y_proj = project_to_obstacles(x_in, y_in, x_obs[j] + vx_obs[j] * total_time, y_obs[j] + vy_obs[j] * total_time, r_obs[j], nsteps)

				for i in range(1, nsteps):
					prediction_time = - i * interval

					x1, y1 = final_pose[0], final_pose[1]
					x2, y2 = x_in, y_in
...
if __name__ == "__main__":
	...
	# lin_u, ang_u, update_var = MPC_solver(init_pose=[0,0,0],current_pose=[.7,.7,0],final_pose=[.7,.7,0],  x_actual=.7, x_destination=.7, x_limit=200, x_origin=0, y_actual = .7, y_destination = .7, y_origin = 0,y_limit = 200 , nsteps=3, interval = .05 ,variables=None, obstacles = [[.5],[.5],[.1],[0],[0]], x_vel_limit = 2, y_vel_limit = 2)
	MPC_solver(init_pose=[0,0,0],current_pose=[.7,.7,0],final_pose=[.7,.7,0], nsteps=4, interval=.2, r_vehicle=.5, obstacles=[[.5],[.5],[.1],[0],[0]])
